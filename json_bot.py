...
bot = TeleBot(token='5504202926:AAGH2iUOJqFfVMQ18L2HZvONfSmlwXIT6zU', parse_mode='html') # создание бота
...
def start_command_handler(message: types.Message):
    ...
    bot.send_message(
        ...
        text=f'Привет {message.from_user.first_name}! Я могу проверить твой JSON и отформатировать его в красивый текст\nВведи JSON в виде строки:', # текст сообщения
        parse_mode='Html')
...
def message_handler(message: types.Message):
    try:
        ...
        bot.send_message(
            ...
            text=f'УПС!!! Соррян, какой то косяк:\n<code>{str(ex)}</code>'
        )
        # выходим из функции
        return
    ...
    bot.send_message(
        chat_id=message.chat.id,
        text=f'Все отлично, ты крут! JSON:\n<code>{text}</code>'
    )
